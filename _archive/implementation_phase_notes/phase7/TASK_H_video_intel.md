# TASK_H: Video Intelligence + ProductionBlueprint

## Model: sonnet | Dependencies: TASK_A, TASK_B complete

Analyze competitor video production style → output ProductionBlueprint for Writer + Media Pipeline.

## Interface

### src/omnicast/analytics/video_intel.py

```python
class VideoIntelligenceAnalyzer:
    """Analyze competitor video production patterns → ProductionBlueprint."""
    def __init__(self, crawler: AnalyticsCrawler)
...
- No actual video download — receives pre-extracted features (transcript, frame descriptions, audio features)
- analyze_* return dicts with specific keys (structure, visual, audio, hook)
- build_blueprint aggregates from top 10% performing videos
- confidence based on sample_size (more videos = higher confidence)
- Must handle < 3 samples gracefully (low confidence, use defaults)

## Tests

### tests/unit/test_video_intel.py
