# TASK_E: Compliance Audit Tab + Telegram Bot Extensions

## Model: sonnet | Dependencies: TASK_A-D complete

Compliance audit trail viewer tab + extend Phase 1 Telegram bot with dashboard commands.

## Interface

### src/omnicast/dashboard/tabs/compliance.py

```python
def render_compliance(data_service: DashboardDataService) -> None:
    """Compliance audit log: search by video_id/channel, filter by pass/fail, date range."""
```

### src/omnicast/telegram/dashboard_commands.py
...
class DashboardCommandHandler:
    ...
    async def cmd_pipeline(self, update, context) -> None
    async def cmd_workers(self, update, context) -> None
    async def cmd_dlq(self, update, context) -> None
    async def cmd_tokens(self, update, context) -> None
    async def cmd_compliance(self, update, context) -> None
    def register(self, application) -> None
```

## DO NOT

- No modifying Phase 1 telegram files — new file dashboard_commands.py only
- Telegram responses must use Markdown formatting
- /dlq shows count + top 3 items only (not full list)
- /tokens must NEVER show token values — show status + expiry only

## Tests

### tests/unit/test_telegram_dashboard.py
