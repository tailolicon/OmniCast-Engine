...
def _vision_verdict(video_path: Path, query: str, world: str = "") -> dict | None:
    ...
    try:
        ...
        prompt = (
            ...
            "handwriting large enough to read. Tiny incidental glyphs (dashboard "
            "digits, radio dial marks, a distant licence plate, blurred lettering) "
            "do NOT count. text_words: the exact words/numbers you can actually "
            "read (empty list if none) — this is the evidence for readable_text. "
            "text_prominent: true when that text is a sign, label, plate, screen, "
            "caption or stamp a viewer would notice and read (a big '124' on a "
            "roadside marker counts; small dashboard digits do not).\n"
            ...
            "contradicts_world: check the VEHICLE CLASS FIRST — if the facts say the "
            "narrator drives a tow truck and the frame shows a semi/tractor-trailer, "
            "dump truck, pickup or bus AS THE NARRATOR'S VEHICLE or as the main "
            "subject, that is true. Other vehicles passing or parked in the "
            "distance/background are normal road scenery — say so in "
            "'contradiction' and answer false. "
            "Otherwise true ONLY for a CLEAR clash with one of these STORY "
            "FACTS — wrong vehicle type, "
            "daylight when the story is at night, wrong era, or a setting the facts "
            "explicitly say never appears. A setting that is merely not mentioned, "
            "an indoor scene, a close-up of an object, or a different angle is NOT a "
            "contradiction. Facts: "
