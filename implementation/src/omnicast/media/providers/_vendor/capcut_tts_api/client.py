...
from omnicast.media.providers._vendor.capcut_tts_api.config import BASE_URL
from omnicast.media.providers._vendor.capcut_tts_api.exceptions import CapCutAPIError, CapCutError, CapCutTaskError
from omnicast.media.providers._vendor.capcut_tts_api.models import DeviceConfig, SubtitleResult, UploadResult, VoiceInfo
from omnicast.media.providers._vendor.capcut_tts_api.signer import (
    base_headers,
    compact_json,
    common_query,
    escape_xml,
...
from omnicast.media.providers._vendor.capcut_tts_api.uploader import VODUploader
...
class CapCutClient:
    ...
    def generate_speech(
        ...
        while time.time() - start_time < timeout:
            ...
            if query_tasks:
                ...
                # CapCut returns "succeed" (not "success") on the SG editor API.
                if status in ("success", "succeed", "done", "finished", "complete"):
                    return query_res
    ...
    def list_voices(
        ...
        """
        # Vendored layout: Voice.json ships beside the package under data/.
        path = catalog_path or Path(__file__).resolve().parent / "data" / "Voice.json"
