...
from omnicast.ingest.douyin._vendor.utils.validators import sanitize_filename
...
# ── Slow-PCDN-node guard (OmniCast addition) ─────────────────────────────────
# Measured on this CDN: a healthy node serves ~6 MB/s on one connection, and
# parallel ranges are SLOWER (per-IP throttling), so re-rolling the redirect is
# the only lever. Floors are deliberately well under a healthy node so an
# ordinary slow connection is never punished.
_SLOW_FLOOR_BPS = float(os.environ.get("OMNICAST_DOUYIN_MIN_BPS", 1_000_000))
_SLOW_WARMUP_S = 6.0            # ignore connection setup and TCP ramp-up
_SLOW_MIN_EXPECTED = 8 * 1024 * 1024   # tiny files are not worth judging
_SLOW_MAX_REROLLS = 3           # then accept whatever speed we can get
_slow_rerolls: dict[str, int] = {}


class SlowNodeError(Exception):
    """Raised to abandon a hopelessly slow CDN node so the retry re-rolls it."""


def _too_slow(final_path, written: int, started: float, expected_size) -> bool:
    if _SLOW_FLOOR_BPS <= 0 or not expected_size or expected_size < _SLOW_MIN_EXPECTED:
        return False
    if _slow_rerolls.get(str(final_path), 0) >= _SLOW_MAX_REROLLS:
        return False
    elapsed = time.monotonic() - started
    if elapsed < _SLOW_WARMUP_S:
        return False
    if written / elapsed >= _SLOW_FLOOR_BPS:
        return False
    _slow_rerolls[str(final_path)] = _slow_rerolls.get(str(final_path), 0) + 1
    return True
        ...
        *,
        ...
        async with aiofiles.open(tmp_path, "wb") as f:
            async for chunk in chunk_iter:
                await f.write(chunk)
                written += len(chunk)
                # The play URL 302s onto a PCDN node and the draw is sometimes
                # a bad one: measured 0.28 MB/s on a link that serves 6 MB/s
                # from a healthy node, turning an 87 MB video into five
                # minutes. Bail out early so the retry re-rolls the redirect
                # instead of crawling to the end.
                if _too_slow(final_path, written, started, expected_size):
                    tmp_path.unlink(missing_ok=True)
                    raise SlowNodeError(
                        f"{written / max(time.monotonic() - started, 0.001) / 1e6:.2f} MB/s "
                        f"from this node, abandoning to re-roll the CDN"
                    )
